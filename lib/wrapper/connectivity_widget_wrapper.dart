...
class ConnectivityWidgetWrapper extends StatefulHookConsumerWidget {
  ...
  @override
  ConsumerState<ConnectivityWidgetWrapper> createState() => _ConnectivityWidgetWrapperState();
}

class _ConnectivityWidgetWrapperState extends ConsumerState<ConnectivityWidgetWrapper> {
  ...
  void _setObservers() {
    streamInternetObservable = InternetConnectionChecker.createInstance().onStatusChange.listen((status) {
      switch (status) {
  ...
  Future<void> _showMaterialBanner() async {
    await NavigationUtils.instance.showInternetIssueScreen(context);
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: theme.pinkBackground,
        ),
        child: Row(
          children: [
            Expanded(
              child: Text(
                _localizations.internetNotConnected,
                style: theme.ts.extTs14.weightBold.copyWith(color: AppColor.themeColorWhite),
              ),
            ),
            Margin.horizontal11,
            const Padding(
